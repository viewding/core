...
export function customizeElement<T extends Constructor<HTMLElement>>(
    ...
    class BaseElement extends superClass {
        ...
        static __attributeNameForProperty(
            name: PropertyKey,
        ...
        defineProperties() {
            for (const [p, propOpt] of Object.entries(
                (this.constructor as typeof BaseElement).properties
            )) {
                // 可以在注解或者静态属性properties中提供实例property的初始值。
                // 否则，如果是通过class的声明语法提供初始值，那么只有在类装饰器中获取。
                const proto = (this.constructor as typeof BaseElement)
                    .prototype;
        ...
        getPropertyOptions(name: PropertyKey) {
            return (this.constructor as typeof BaseElement).properties[
                name.toString()
            ];
        }
        ...
        constructor(...args: any[]) {
            ...
            this.innateStyles = reactive({}) as Record<
                string,
                string | number | undefined | null
            >;
